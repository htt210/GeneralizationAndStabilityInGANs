...
class ContinualClassifier(nn.Module):
    ...
    def _compute_fisher(self, real_list, fake_list, ones, zeros, batch_mode=True):
        est_fisher_info = {}
        for n, p in self.named_parameters():
            if p.requires_grad:
        ...
        est_fisher_info = {n: p / (2 * len(fake_list)) for n, p in est_fisher_info.items()}
...
class ContinualCritic(nn.Module):
    def __init__(self, nx, nhidden, nlayers, discount):
        super(ContinualCritic, self).__init__()
        self.net = nn.Sequential()
        self.net.add_module('input', nn.Linear(nx, nhidden))
        self.net.add_module('act0', nn.ReLU(True))
        for i in range(nlayers):
...
def GAN_GP(D, G, data, noise, nc, img_size, niter=10000, d_steps=1, batch_size=32,
    ...
    for iter in range(niter):
        ...
        if iter % args.interval == args.interval - 1:
            ...
            D._compute_fisher(buffer_real, buffer_fake, ones, zeros, args.batch_mode)
            buffer_fake = []
            buffer_real = []
...
def WGAN_GP(D: ContinualCritic, G: Generator, data, noise, nc, img_size, niter=10000, d_steps=5, batch_size=32,
    ...
    for idx in range(niter):
        if idx % 1000 == 0:
            print(datetime.datetime.now() - start, idx)
            with torch.no_grad():
                fake_batch = G(fixed_z)
                imgs = fake_batch.data.resize(64, nc, img_size, img_size)
                torchvision.utils.save_image(imgs, prefix + '/%s_iter_%06d.png' % (args.dataset, idx))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--discount', type=float, default=0.5, help='discount factor for old Fisher info')
