...
def GAN_GP(D: ContinualClassifier, G: Generator, data, noise, niter=10000, batch_size=32, optimizer='Adam',
    ...
    for iter in range(niter):
        if iter % 100 == 0:
            ...
            # acc_ax.scatter(fake_batch[:, 0], fake_batch[:, 1], s=2, c='r', marker='+')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--discount', type=float, default=0.9999, help='discount factor for old Fisher info')
    ...
    prefix = 'figs/continual_%s_%s_gradfield_center_%.2f_alpha_%s_lambda_%.2f_lrg_%.5f_lrd_%.5f_nhidden_%d_scale_%.2f' \
             '_optim_%s_gnlayers_%d_dnlayers_%d_gradweight_%.4f_discount_%.4f_batch_%d_ewcweight_%.2f_ncritic_%d/' % \
